...
def test_guest_can_go_to_catalog(walker):
    walker.go_to_url()
    walker.click_element(TestSite.BUTTON_GO_TO_CATALOG)
    assert walker.text_of_element(TestSite.IS_CATALOG) == 'Women', "[-] It's not 'Catalog'"
...
class TestCatalog:
    ...
    def test_guest_can_click_category(self, walker):
        walker.go_to_url()
        walker.click_element(TestSite.BUTTON_GO_TO_CATALOG)
        walker.click_element(TestSite.CATEGORY_TOPS)
    ...
    def test_guest_sees_store_information(self, walker):
        walker.go_to_url()
        walker.click_element(TestSite.BUTTON_GO_TO_CATALOG)
        found_matches = walker.find_text_in_element_by_regex(TestSite.STORE_INFORMATION,
                                                             r'[(][0-9]{3}[)] [0-9]{3}-[0-9]{4}')
